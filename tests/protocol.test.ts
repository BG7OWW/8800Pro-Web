...
import { getShx8800ProBluetoothReadWriteAddresses, getShx8800ProReadWriteAddresses, SHX8800PRO } from '../src/core/constants/memory-map'
...
assert.equal(officialWrites.length, getShx8800ProBluetoothReadWriteAddresses().length)
...
class LegacyBluetoothWriteTransport implements RadioTransport {
  readonly kind = 'bluetooth' as const
  readonly label = 'legacy-ble-write'
  readonly writes: Uint8Array[] = []
  ackReads = 0
  private queue: number[] = []
  private streamPayloads = 0
  private configFrames = 0
  ...
  async write(data: Uint8Array) {
    ...
    if (data.length === 4 && data[0] === 0x57 && data[3] === 0x40) {
      this.streamPayloads = 0
      return
    }
    if (data.length === SHX8800PRO.framePayloadBytes) {
      this.streamPayloads += 1
      if (this.streamPayloads === 2) this.queue.push(0x06)
      return
    }
    if (data.length === SHX8800PRO.frameBytes && data[0] === 0x57 && data[3] === 0x40) {
      this.configFrames += 1
      if (this.configFrames % 2 === 0) this.queue.push(0x06)
      return
    }
    if (data.length === 1 && data[0] === 0x45) this.queue.push(0x06)
  }
...
const legacyBluetoothWriteTransport = new LegacyBluetoothWriteTransport()
await new Shx8800ProSession(legacyBluetoothWriteTransport, {
  bluetoothWriteMode: 'legacy',
  bluetoothWritePairDelayMs: 0,
  bluetoothAckSettleMs: 0,
}).writeRadio(bluetoothWriteData)
const legacyHeaderIndex = legacyBluetoothWriteTransport.writes.findIndex((write) => write.length === 4 && write[0] === 0x57 && write[3] === 0x40)
assert.ok(legacyHeaderIndex >= 0)
assert.deepEqual(Array.from(legacyBluetoothWriteTransport.writes[legacyHeaderIndex]), [0x57, 0x00, 0x00, 0x40])
assert.equal(legacyBluetoothWriteTransport.writes.some((write) => write.length === SHX8800PRO.bluetoothFrameBytes && write[0] === 0x57), false)
assert.equal(legacyBluetoothWriteTransport.ackReads, 1 + Math.ceil(getBluetoothWriteBlocks(bluetoothWriteData).length / 2))
