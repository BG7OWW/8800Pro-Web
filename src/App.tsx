  ...
  return (
    <section className="panel about-panel">
      ...
      <div className="about-copy">
        ...
        <p>蓝牙写频则更像一次实机考古。网页的 Web Bluetooth、macOS CoreBluetooth 脚本、APK 行为和真实对讲机响应被放在一起比对：先确认设备广播名和 FFE0/FFE1 特征值，再验证 BLE 下仍然使用同一套写频帧，只是传输要拆成小包。后来遇到第 70 个信道附近重启、空信道冒出随机频率等问题，也是靠实机读回、逐块比较和保留现场日志才定位到空块必须写成干净的 `FF` 填充，而 404.00857 这类频率则暴露出帧头被机器当成数据区写入的问题，所以蓝牙写帧改成帧内连续流式发送，尽量贴近实机验证过的 CoreBluetooth 写入节奏。</p>
        ...
        <div className="changelog">
          <p><strong>蓝牙帧内流式写入</strong> 404.00857 这类乱码频率对应 `57 addr 40` 写帧头，BLE 写频改为 18 字节小包连续无响应发送，避免浏览器等待 GATT 响应时把一个 68 字节帧拖断。</p>
